...
const STRIPE_REFRESH_URL = 'https://wetalk247.tv/dev/';
const STRIPE_RETURN_URL = 'https://wetalk247.tv/dev/';
...
export default class StripeService {
  static createUserStripeAccounnt = async (email: string, userId: string): Promise<IAddUserAccount | undefined> => {
    try {
  ...
  static async getPaymentUrl(title: string, price: number, userId: string) {
    try {
  ...
  static chargeCard = async (checkout: ICheckout, tokenId: string, user: IUsers) => {
    try {
      const idempotencyKey = uuidv4();
...
//   static async gettingURL(
//     req: { query: { sessionId: any } },
...
//           },
//         ],
//         //   success_url: `${process.env.APP_HOST}/success?payment`,
//         //   cancel_url: `${process.env.APP_HOST}/cancel.html`,
//         success_url: `${process.env.SERVER_HOST}/payment?success=true&session=${session.id}`,
//         cancel_url: `${process.env.SERVER_HOST}/payment?success=false&session=${session.id}`,
//       });
//     } catch (e) {
//       debugLogError1(e);
//     }
...
//   static async getPaymentUrl_old(req: any, res: { json: (arg0: any) => void }) {
//     debugLog1("In function PaymentController.getPaymentUrl");
...
//           },
//         ],
//         success_url: `${process.env.SERVER_HOST}/payment?success=true`,
//         cancel_url: `${process.env.SERVER_HOST}/payment?success=false`,
//       });

//       const httpResponse = HttpResponse.get({ url: session.url });
//       res.json(httpResponse);
//     } catch (e) {
//       res.json(new BadRequest(`error: ${e.message}`));
//     }
//   }
// }
